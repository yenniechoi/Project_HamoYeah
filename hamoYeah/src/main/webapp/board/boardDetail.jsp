...
<body>
	...
	<div class="H-cantainer">
		...
		<div class="HB_btn">
			<c:if
				test="${sessionScope.loginId eq boardvo.memberId && boardvo.y_card < 3}">
				<input type="button" value="수정"
					onclick="javascript:location.href='${pageContext.request.contextPath}/board/editBoard.do?boardNum=${boardvo.boardNum }'">
				<input type="button" value="삭제"
					onclick="delBoard('${boardvo.boardNum}', '${sessionScope.loginId}')">
			</c:if>
		</div>
						...
						<div class="content">
							<form action="" method="post" name="f">
								<input type="hidden" name="memberId"
									...
									name="boardNum" id="${boardvo.boardNum}"
									value="${boardvo.boardNum}"> <input type="radio"
									name="content" id="warn" value=" 혐오 발언 또는 상징"
									onclick="this.form.textarea.disabled=true">혐오 발언 또는 상징<br />
								<input type="radio" name="content" id="warn" value=" 폭력적인 게시글"
									onclick="this.form.textarea.disabled=true">폭력적인 게시글<br />
								<input type="radio" name="content" id="warn"
									value=" 불법 또는 규제 상품 판매"
									onclick="this.form.textarea.disabled=true">불법 또는 규제 상품
								판매<br /> <input type="radio" name="content" id="warn"
									value=" 이미지 도용" onclick="this.form.textarea.disabled=true">이미지
								도용<br /> <input type="radio" name="content" id="warn"
									value=" 자살 또는 자해 유도성 게시글"
									onclick="this.form.textarea.disabled=true">자살 또는 자해 유도성
								게시글<br /> <input type="radio" name="content" id="warn"
									value=" 사기 또는 거짓" onclick="this.form.textarea.disabled=true">사기
								또는 거짓<br /> <input type="radio" name="content" id="warn"
									value=" 정치적 또는 종교적 성향 게시글"
									onclick="this.form.textarea.disabled=true">정치적 또는 종교적
								성향 게시글<br /> <input type="radio" name="content" id="warnetc"
									value="6" onclick="this.form.textarea.disabled=false">기타<br />
								<textarea name="textarea" id="etc" cols="37" rows="4"
									style="scrolling: yes; resize:none;" placeholder="기타 의견을 입력해 주세요. (최대 100자)" maxlength="100" disabled></textarea><br/>
								<input type="button" class="WH_btn" value="신고" id="reportbtn2" style="font-family: 'NanumSquareNeoLight';"
									onclick="reportBoard()">
							</form>
						</div>
					</div>
	...
	<div class="container">
		<div id="content-board">
			<!-- boardvo 등록된 게시글 이미지 -->
			<div class="B_img ">
				<img src="${boardvo.imagepath }"
					style="width: 880px; height: 350px; padding-left: 8%; padding-right: 8%">
			</div>
			<br/>
			<!-- boardvo 제목 -->
			<div id="" style="text-align:center;">
			<div class="B_title">
				<div class="title" style="text-align:center; font-weight:bold; color:#52c8bd;">${boardvo.title }</div>
			</div>
			<br/>
		...
		<div class="content-host">
			<div class="content-H_ost" style="text-align:center; margin-left:46%;">
				<div id="host">
					<div class="ho" style="font-size: 15px;">주최자</div>
					<a onclick="userinfo('${membervo.memberId}')">
					<img src="${membervo.imagepath }" style="width: 50px; height: 50px; border-radius:70%;"></a>
					<!-- membervo 프로필사진  -->
		...
		<div class="content-participate" style="text-align:center; margin-right:11.5%">
			...
			<div id="P_ok" style="text-align:center">
				<c:forEach var="mvo" items="${boardvo.mvolist}">
					<span id="${mvo.memberId }"  onclick="userinfo('${mvo.memberId}')">
						<img src="${mvo.imagepath }" style="width: 50px; height: 50px; border-radius: 70%">
						${mvo.nickname }
					</span>
				</c:forEach>
			</div>
<br/><br/><br/>
			<div class="PH_btn text-align:center;">
				...
				<c:if
					test="${not empty sessionScope.loginId && sessionScope.loginId ne boardvo.memberId && off eq 0}">
					<!-- 참여 신청 안함 + 인원수 다 안참 + 신고 누적 3미만  -->
					<c:if
						test="${membervo.participate eq 0 && boardvo.ok ne boardvo.peopleMax && boardvo.y_card ne 3}">
						<input type="button" value="참여신청" class="shadow bg-blue-400 hover:bg-blue-200 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-6 rounded" 
							onclick="par('${boardvo.boardNum}', '${sessionScope.loginId}')"
							style="background-color: #52c8bd; border:none; font-weight:bold;">
					</c:if>
					<!-- 참여 신청을 이미 했다면? 혹은 승인됐지만 신청 취소하고 싶다면? -->
					<c:if
						test="${membervo.participate eq 1 || membervo.participate eq 2}">
						<input type="button" value="신청취소" class="shadow bg-blue-400 hover:bg-blue-200 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-6 rounded" 
							onclick="par('${boardvo.boardNum}', '${sessionScope.loginId}')"
							style="background-color: lightgrey; border:none; font-weight:bold;">
					</c:if>
					<!-- 참여 신청 안했는데 최대 인원이 다 찼다면? + 신고누적 3미만-->
					<c:if
						test="${membervo.participate eq 0 && boardvo.ok eq boardvo.peopleMax && boardvo.y_card ne 3}">
						<input type="button" value="신청마감" onclick="alert('신청이 마감되었습니다.')" class="shadow bg-blue-400 hover:bg-blue-200 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-6 rounded" 
							style="background-color: grey; border:none; font-weight:bold;">
					</c:if>
				</c:if>
			</div>
		</div>
